...
# 六.Spring Boot 中的两种 JSON 序列化机制



在 Spring Boot 项目中，将 Java 对象转换为 JSON 字符串的操作非常常见，但根据目标的不同，其背后的机制和配置也完全不同。我们主要关注“**返回给前端**”和“**存入 Redis**”这两种场景。
...
- **负责人 (组件)**：`MappingJackson2HttpMessageConverter`
  - 这是 Spring MVC 框架的一部分，负责处理 `@RestController` 或 `@ResponseBody` 注解的方法返回值。
- **工作目标 (Goal)**：
  - 将 Java 对象（如 `List<DishVO>`）转换成 JSON 字符串，这个 JSON 字符串只要能被**前端的 JavaScript 正确解析**即可。
  - 它**不需要**关心这个 JSON 未来是否需要再转回**一模一样**的 Java 对象。
- **配置与行为 (Behavior)**：
  - Spring Boot 已经为这个转换器**预先配置**好了一个非常智能的 `ObjectMapper` (Jackson 库的核心)。
  - 这个预配置的 `ObjectMapper` **知道**如何处理 `java.time.LocalDateTime` 等 Java 8 的新时间类型，会将其转换为标准的字符串格式。
  - 它在序列化时**不会**添加额外的 Java 类型信息（如 `@class` 字段），因为前端 JavaScript 不需要这些信息。
- **结论**：这个流程通常是“开箱即用”的，因为 Spring Boot 已经为我们处理好了大部分常见类型的转换。



## 二、 序列化场景 2：将数据存入 Redis



- **负责人 (组件)**：`RedisTemplate` 及其配置的 `RedisSerializer`
  - 在我们的项目中，我们为其配置了 `GenericJackson2JsonRedisSerializer`。
- **工作目标 (Goal)**：
  - 这个序列化器的目标是**可逆的**。它不仅要把 Java 对象变成 JSON 字符串存进去，更重要的是，未来要能从这个 JSON 字符串**完美地、无损地恢复**出原始的 Java 对象（包括其具体的泛型类型）。
- **配置与行为 (Behavior)**：
  - 为了实现“无损恢复”，`GenericJackson2JsonRedisSerializer` 默认会采取一种**“写入类型信息”**的策略。它会在生成的 JSON 中，额外加入一个类似 `@class` 的字段，来记录这个对象的原始 Java 类型（例如 `com.sky.vo.DishVO`）。
  - 默认情况下，`GenericJackson2JsonRedisSerializer` 内部的 `ObjectMapper` 配置比较“原始”，它**不知道**如何为 `LocalDateTime` 这种非基本类型添加“类型信息”。
...
| 特性         | **返回给前端 (Spring MVC)**                        | **存入 Redis (`RedisTemplate`)**                             |
| ------------ | -------------------------------------------------- | ------------------------------------------------------------ |
| **目标**     | **单向**，前端能用即可                             | **双向**，必须能无损恢复成 Java 对象                         |
| **类型信息** | **不包含** Java 类型信息                           | **默认会包含**额外的 `@class` 等类型信息                     |
| **配置**     | Spring Boot **已预先配置好**，支持 `LocalDateTime` | **需要手动配置** `ObjectMapper`，为其开启类型处理并注册时间模块 |
| **结果**     | 通常能直接工作                                     | 在处理 `LocalDateTime` 等复杂类型时，若不额外配置则会报错    |

**最终结论**：虽然都是“序列化”，但因为“存入 Redis”的场景比“返回给前端”多了一个“**未来要能完美恢复**”的苛刻目标，所以它需要更详细、更明确的配置才能正确工作。
...
### **七、 Spring Task 定时任务框架**



Spring Task 是 Spring 框架自身提供的一个轻量级定时任务框架。它允许开发者通过简单的注解，在指定的时间点或按固定的频率，自动地执行某个方法，非常适合执行一些周期性的后台任务。
...
为了让定时任务生效，Spring Task 提供了两个核心注解：

- **`@EnableScheduling`**
  - **作用**：这是一个“**总开关**”。您必须将这个注解添加在项目的主启动类或任意一个配置类上，来**开启**基于注解的定时任务功能。
  - **原理**：一旦开启，Spring 就会在后台创建一个任务调度器，并开始扫描项目中所有被 `@Scheduled` 注解标记的方法。
- **`@Scheduled`**
  - **作用**：这个注解用在**方法**上，标记这个方法是一个需要被定时调度的任务。它提供了多种参数来定义任务的执行时机。
...
@EnableScheduling // 2. 在这里添加注解，开启定时任务
...
Cron 表达式是一个字符串，由 6 个或 7 个用空格分隔的域组成，用于精确地定义任务的执行时间。

- **标准 Cron 表达式（6个域）**： `秒 分钟 小时 日期 月份 星期`
- **Spring 支持的 Cron 表达式（包含可选的年份）**： `秒 分钟 小时 日期 月份 星期 [年份]`
...
| **星期** | Day of Week  | 1-7 或 SUN-SAT (1=SUN) | `*` `/` `,` `-` `?` `L` `#` |
| **年份** | Year         | 1970-2099              | `*` `/` `,` `-` (可选)      |

**特殊字符含义：**

- **`\*`**: 代表“每一个”。例如，在“分钟”域使用 `*`，表示每分钟都会触发。
- **`?`**: 只能用在“日期”和“星期”域，表示不指定值。用于解决“日期”和“星期”的冲突（例如，你不能同时指定“5号”并且是“星期三”）。
- **`-`**: 表示范围。例如，在“小时”域使用 `10-12`，表示 10点、11点和12点。
- **`,`**: 表示枚举值。例如，在“分钟”域使用 `0,15,30,45`，表示在 0、15、30、45 分时触发。
- **`/`**: 表示步长。例如，在“秒”域使用 `0/5`，表示从 0 秒开始，每隔 5 秒触发一次。
...
WebSocket 是一种允许服务器与客户端建立持久的双向通信通道的网络协议，它通过一个持续的 [TCP 连接](https://www.google.com/search?sca_esv=c8099ca82f4112b0&sxsrf=AE3TifMM89cnnQoW8ZFa6YLr1NH44lutgg%3A1760082826033&q=TCP+连接&sa=X&sqi=2&ved=2ahUKEwiSwvu8k5mQAxXvZvUHHTRjBngQxccNegQIKRAC&mstk=AUtExfB1lKaMWeWjIiHrR6-kdW_WBdz_t6TPEX_lui5P1SffGSD0iOdIrEOg6hrw-oGDLKidmbUzd1lVJT9V3y5t9Dy5dChtmv9lb5B_K-9fTTL9HtvYexUJd1mY5hvWKpISQcam3PiJhtnzUgRfwdCugR96MDsyzISj48BaS1bZXppmzbg&csui=3)来实现实时数据交换，而无需客户端不断地向服务器发送请求。 它在实现实时应用（如在线游戏、聊天应用和股票交易平台）时特别有用，因为它可以减少网络流量和延迟，显著提高效率。 
...
![在这里插入图片描述](D:\porjects\Food_Delivery\Food_Delivery\md\图片\ed665c42afe349109f9d9b1e3dbc36bf.png)
...
1.springboot项目调试接口`submit`就会The application is running ，不调试就没事，而且请求过来立马放行也这样

但是数据库信息也入库，前端信息请求正常返回



2.前端传来的口味数据不排序， 重辣 不要蒜  和  不要蒜 重辣，返回给后端数据不一致，判断加购物车是选择新加一个还是数量加一时出现逻辑问题
...
4.order类的两个属性不是包装类，所以默认值为0，在更新数据时，把数据给污染了

5.订单管理， 当申请为page =2 则去查询待接单的数据 page也为2，由于没有第二页所以返回空值

6.在“再来一单”的流程中，无将旧 `order_detail` 复制到 `shopping_cart`，，代码**错误地将旧记录的主键 `id` 也一并复制了过去**。

因此，在准备向 `order_detail` 表插入新记录时，这个新的 `OrderDetail` 对象携带的是一个已经存在于数据库中的旧 ID。





7.前端催单时，不会重新加载，导致用户可以在同一界面多次点催单           



当小程序的请求地址改为服务器，记得留意服务器nginx是否会反向解析后端的websocket服务
...
![image-20250920140651037](D:\porjects\Food_Delivery\Food_Delivery\md\图片\aa.png)
